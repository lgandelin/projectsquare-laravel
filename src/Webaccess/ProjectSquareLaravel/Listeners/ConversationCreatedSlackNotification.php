...
class ConversationCreatedSlackNotification
{
    ...
    {
        if ($conversation = (new EloquentConversationRepository())->getConversationModel($event->conversation->id)) {

            $lines = [
                (isset($conversation->project) && isset($conversation->project->client)) ? 'Projet : *[' . $conversation->project->client->name . '] ' . $conversation->project->name . '*' : '',
