<form action="{{ route('tickets_update') }}" method="post">
    <div class="row ticket-state">
        <div class="col-md-6">
            ...
            <div class="form-group">
                <label for="comments">{{ __('projectsquare::tickets.comments') }}</label>
                <textarea class="form-control" rows="4" placeholder="{{ __('projectsquare::tickets.comments') }}" name="comments"></textarea>
            </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input class="form-control datepicker" type="text" placeholder="{{ __('projectsquare::tickets.due_date_placeholder') }}" name="due_date" @if (isset($ticket->states[0]->dueDate))value="{{ $ticket->states[0]->dueDate }}"@endif autocomplete="off" />
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="priority">
                    <option value="">{{ __('projectsquare::generic.choose_value') }}</option>
                    @for ($i = 1; $i <= 3; $i++)
                    <option value="{{ $i }}" @if (isset($ticket) && isset($ticket->states[0]) && $ticket->states[0]->priority == $i)selected="selected"@endif>{{ __('projectsquare::generic.priority-' . $i) }}</option>
                    @endfor
                </select>
            </div>
            ...
            @if (!$is_client)
                <div class="form-group">
                    <div style="display: inline-block">
                        ...
                        <input class="form-control" type="text" name="estimated_time_hours" style="display: inline-block; width: 6rem; margin-left: 1rem; margin-right: 0.5rem;" value="@if (isset($ticket) && isset($ticket->states[0]) && isset($ticket->states[0]->estimatedTimeHours)){{ $ticket->states[0]->estimatedTimeHours }}@endif" /> {{ __('projectsquare::generic.hours') }}
                    </div>

                    <div style="display: inline-block; margin-left: 5rem;">
                        <label for="estimated_time">{{ __('projectsquare::tasks.spent_time') }}</label><br>
                        <input class="form-control" type="text" name="spent_time_hours" style="display: inline-block; width: 6rem; margin-left: 1rem; margin-right: 0.5rem;" value="@if (isset($ticket) && isset($ticket->states[0]) && isset($ticket->states[0]->spentTimeHours)){{ $ticket->states[0]->spentTimeHours }}@endif" /> {{ __('projectsquare::generic.hours') }}
                    </div>
                </div>
            @endif
        </div>
